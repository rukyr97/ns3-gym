...
def build_ns3_project(debug=True):
    ...
    buildRequired = False
    ns3Proc = subprocess.Popen(
        wafString, shell=True, stdout=subprocess.PIPE, stderr=None, universal_newlines=True)
...
def start_sim_script(port=5555, simSeed=0, simArgs={}, debug=False, src_dir=os.getcwd()):
    simScriptName = os.path.basename(src_dir)
    wafPath = find_waf_path(src_dir)
    baseNs3Dir = os.path.dirname(wafPath)
    ...
    if debug:
        ns3Proc = subprocess.Popen(
            wafString, shell=True, stdout=None, stderr=None)
    else:
        '''
        ...
        ns3Proc = subprocess.Popen(
            wafString, shell=True, stdout=subprocess.PIPE, stderr=errorOutput, universal_newlines=True)
